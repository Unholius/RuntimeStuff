...
namespace RuntimeStuff.Extensions
{
    ...
    public static class RSDataTableExtensions
    {
        ...
        /// <returns></returns>
        public static List<T> ToList<T>(this DataTable dt, IDictionary<string, string> mapper = null, bool fastSet = false) where T : class, new()
        {
            if (dt == null)
            ...
            foreach (DataRow row in dt.Rows)
            {
                ...
                {
                    if (fastSet)
                        col.Item2.SetValue(item, row[col.Item1]);
                    else
