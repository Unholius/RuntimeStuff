...
///// <summary>
/////     Статический вспомогательный класс для расширенной работы с объектами, их свойствами, полями, методами и типами в
/////     .NET с использованием рефлексии.
/////     <para>
/////         <b>Основные возможности:</b>
/////         <list type="bullet">
/////             <item>
/////                 — Универсальный доступ к значениям свойств и полей по имени или пути (в том числе вложенным и
/////                 индексированным), с возможностью преобразования типов.
/////             </item>
/////             <item>
/////                 — Гибкое преобразование типов, поддержка пользовательских конвертеров, безопасные и
/////                 fallback-преобразования.
/////             </item>
/////             <item>
/////                 — Копирование и слияние свойств между объектами, включая глубокое копирование, работу с коллекциями и
/////                 словарями, а также правила перезаписи значений.
/////             </item>
/////             <item>
/////                 — Динамический вызов методов по имени, получение и установка значений через
/////                 <see cref="MemberInfoEx" />.
/////             </item>
/////             <item>— Получение атрибутов для объектов и их членов, расширенная работа с метаданными .NET.</item>
/////             <item>
/////                 — Универсальное создание экземпляров объектов, поддержка DataRow/DataRowView, кортежей, перечислений
/////                 и других специальных случаев.
/////             </item>
/////             <item>— Сжатие и распаковка массивов байт с помощью GZip, копирование потоков.</item>
/////             <item>
/////                 — Кэширование результатов операций Split и рефлексии для повышения производительности при повторных
/////                 вызовах.
/////             </item>
/////         </list>
...
/////         <list type="bullet">
/////             <item>
/////                 — Маппинг и преобразование данных между различными объектными моделями (DTO, ORM,
/////                 сериализация/десериализация).
/////             </item>
/////             <item>
/////                 — Динамическое связывание данных в UI, редакторы свойств, построение универсальных инспекторов
/////                 объектов.
/////             </item>
/////             <item>— ETL, интеграция, обработка данных, где требуется универсальный доступ к структурам объектов.</item>
/////             <item>
/////                 — Генерация кода, скриптовые движки, плагин-системы, где необходима работа с объектами на этапе
/////                 выполнения.
/////             </item>
/////         </list>
/////         <b>Потокобезопасность:</b> Все статические члены класса потокобезопасны. Для кэширования и пользовательских
/////         конвертеров используются concurrent-коллекции.<br />
/////         <b>Производительность:</b> Класс оптимизирован для частого использования в динамических сценариях за счёт
/////         кэширования и эффективных паттернов работы с рефлексией.
/////     </para>
///// </summary>
...
//    public static MemberInfoEx GetMember(Type type, string memberName,
//        MemberNameType memberNameType = MemberNameType.Any)
//    {
//        if (type == null)
...
//                return toType.Default();
//            if (toType.IsEnum)
//                return Enum.ParseDate(toType, s, true);
...
//    /// <param name="formatProvider">Провайдер формата.</param>
//    /// <returns>True, если преобразование успешно; иначе false.</returns>
//    public static bool TryChangeType(object value, Type[] toTypes, out object convert,
...
//    /// <param name="formatProvider">Провайдер формата.</param>
//    /// <returns>True, если преобразование успешно; иначе false.</returns>
//    public static bool TryChangeType(object value, Type toType, out object convert,
//        IFormatProvider formatProvider = null)
//    {
//        try
//        {
...
//        int cnt;
//        while ((cnt = src.Read(bytes, 0, bytes.Length)) != 0) target.Write(bytes, 0, cnt);
//    }
...
//    public static object Get(object obj, string propName, Type convertToType = null,
//        MemberNameType memberNameType = MemberNameType.Any)
//    {
...
//    /// <returns>Значение свойства или null.</returns>
//    public static object Get(object obj, string[] propertyNames, Type convertToType = null,
//        MemberNameType memberNameType = MemberNameType.Any,
...
//                var objMember = objTypeInfo.GetMember(propertyNames[0], memberNameType,
//                    x => x.IsProperty || x.IsField || x.IsMethod, nameComparison);
...
//                    : Get(objMember.Getter(obj), propertyNames.Skip(1).ToArray(), convertToType, memberNameType,
//                        nameComparison);
//        }
//    }
...
//    public static bool Set(object obj, string propName, object value,
//        MemberNameType memberNameType = MemberNameType.Any)
//    {
...
//    public static bool Set(object obj, string[] propertyNames, object value,
//        MemberNameType memberNameType = MemberNameType.Any,
//        StringComparison nameComparison = StringComparison.OrdinalIgnoreCase)
//    {
//        if (obj == null || !propertyNames.Any())
...
//                var propertyOrField = objTypeInfo.GetMember(propertyName, memberNameType,
//                    x => x.IsProperty || x.IsField, nameComparison);
...
//    /// <summary>
//    ///     Устанавливает значение свойства или поля объекта по имени с учетом регистра с минимальным количеством проверок.
//    ///     <br />
//    ///     Самый быстрый способ установить значение свойства или поля объекта это <see cref="MemberInfoEx.Setter" />.<br />
//    ///     Не конвертирует тип значения в тип свойства.<br />
...
//    {
//        var objTypeInfo = obj.GetType().GetMemberInfoEx();
//        var propertyOrField = objTypeInfo.GetMember(propertyName, MemberNameType.Name, x => x.IsProperty || x.IsField,
...
//    public static void Copy(object source, object target, bool deepProcessing = false,
//        StringComparison nameComparison = StringComparison.OrdinalIgnoreCase)
//    {
...
//    /// <param name="target">Целевой объект.</param>
//    /// <param name="deepProcessing">Выполнять глубокое копирование (создавать новые экземпляры для ссылочных типов)</param>
//    /// <param name="overwritePropertiesWithNullValues">Перезаписывать в source свойства null значениями из target.</param>
//    /// <param name="overwriteOnlyPropertiesWithDefaultValue">Перезаписывать только null свойства в source.</param>
//    /// <param name="memberNameType">Тип имени члена.</param>
//    /// <param name="nameComparison">Сравнение имен</param>
//    /// <param name="mapper">Сопоставление имен source -> target</param>
//    public static void Copy(object source, object target, bool deepProcessing, bool overwritePropertiesWithNullValues,
//        bool overwriteOnlyPropertiesWithDefaultValue, MemberNameType memberNameType = MemberNameType.Name,
//        StringComparison nameComparison = StringComparison.OrdinalIgnoreCase, Dictionary<string, string> mapper = null)
//    {
...
//        {
//            var srcItemType = srcType.GetElementType() ??
//                              srcType.GenericTypeArguments.FirstOrDefault() ?? typeof(object);
...
//            {
//                if (targetItemType == typeof(object) || targetItemType.IsImplements(srcItemType) ||
//                    (srcItemTypeInfo.IsValueType && targetItemTypeInfo.IsValueType))
//                {
...
//                    Copy(i, targetListItem, deepProcessing, overwritePropertiesWithNullValues,
//                        overwriteOnlyPropertiesWithDefaultValue, memberNameType, nameComparison);
//                    if (targetType.IsArray)
//                        ((Array)target).SetValue(targetListItem, idx);
//                    else
//                        Call(target, nameof(IList.Add), targetListItem);
...
//            if (overwriteOnlyPropertiesWithDefaultValue)
//            {
...
//                    !EqualityComparer<object>.Default.Equals(targetPropValue, targetPropValue.GetType().Default()))
//                    continue;
//            }

//            var castedValue = Cast(srcPropValue, tm.PropertyType, deepProcessing, nameComparison);
//            Set(target, new[] { mapper?.Any() == true ? mapper.GetValueOrDefault(tm.Name, tm.Name) : tm.Name },
//                castedValue, memberNameType, nameComparison);
//        }
//    }
...
//    public static T Cast<T>(object objInstance, bool deepProcessing,
//        StringComparison nameComparison = StringComparison.OrdinalIgnoreCase, object[] ctorArgs = null)
//    {
...
//    public static object Cast(object objInstance, Type toType, bool deepProcessing,
//        StringComparison nameComparison = StringComparison.OrdinalIgnoreCase, object[] ctorArgs = null)
//    {
//        if (objInstance == null)
//            return null;
...
//    public static Dictionary<string, object> GetMembersValues(object obj, Func<MemberInfoEx, bool> matchCriteria,
//        params object[] args)
//    {
...
//    public static MemberInfoEx GetMethodInfo(Type type, string methodName,
//        StringComparison stringComparison = StringComparison.OrdinalIgnoreCase)
//    {
//        var mi = type.GetTypeInfo();
//        return mi.GetMemberInfoEx()?.Members
//            .FirstOrDefault(x => x.IsMethod && x.Name.Equals(methodName, stringComparison));
//    }
...
//    /// <param name="stringComparison">Способ сравнения имен.</param>
//    /// <returns>Экземпляр <see cref="MemberInfoEx" /> или null, если метод не найден.</returns>
//    public static MemberInfoEx GetMethodInfo<T>(string methodName,
//        StringComparison stringComparison = StringComparison.OrdinalIgnoreCase)
//    {
...
//    /// <param name="stringComparison">Способ сравнения имен.</param>
//    /// <returns>Экземпляр <see cref="MemberInfoEx" /> или null, если метод не найден.</returns>
//    public static MemberInfoEx GetMethodInfo<T>(T obj, string methodName,
//        StringComparison stringComparison = StringComparison.OrdinalIgnoreCase)
//    {
