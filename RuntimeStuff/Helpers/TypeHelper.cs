...
public static class TypeHelper
{
    ...
    private static readonly Converter<string, DateTime?> StringToDateTimeConverter = s =>
    {
        ...
        if (DateTime.TryParseExact(s.Trim(), _dateFormats, CultureInfo.InvariantCulture, styles, out var result))
            return result;
    ...
    {
        if (value == null || (value?.Equals(DBNull.Value) != false && IsNullable(toType)))
            return null;
    ...
    public static Type[] GetImplementationsOf(Type baseType, Assembly fromAssembly = null)
    {
    ...
    public static Type GetTypeByName(string typeOrInterfaceName)
    {
        ...
        // Проверяем кэш
        if (_typeCache.TryGetValue(typeOrInterfaceName, out var cachedType))
            return cachedType;
        ...
        _typeCache[typeOrInterfaceName] = foundType;
