...
namespace System.Linq
{
    ...
    public static class EnumerableExtensions
    {
        ...
        /// <exception cref="NullReferenceException">source.</exception>
        /// <exception cref="NullReferenceException">predicate.</exception>
        ...
        /// <exception cref="InvalidOperationException">Коллекция не поддерживает TryClear.</exception>
        ...
        public static IList<T> FromCsv<T>(this IList<T> toList, string csv, params Expression<Func<T, object>>[] propertySelectors)
            where T : class, new()
        {
            var items = CsvHelper.FromCsv<T>(csv, propertySelectors.Select(x => x.GetPropertyInfo()).ToArray());
        ...
        /// в противном случае — <c>-1</c>.</returns>
        /// <exception cref="ArgumentNullException">e.</exception>
        /// <exception cref="ArgumentOutOfRangeException">fromIndex - fromIndex не может быть меньше нуля.</exception>
        ...
        /// <summary>
        /// Преобразует последовательность <see cref="IEnumerable{T}"/> в <see cref="ObservableCollection{T}"/>.
        /// Для каждого элемента создаётся новый объект и копируются его свойства.
        /// </summary>
        /// <typeparam name="T">Тип элементов исходной последовательности и коллекции.</typeparam>
        /// <param name="source">Исходная последовательность.</param>
        /// <returns>
        /// Коллекция <see cref="ObservableCollection{T}"/>, содержащая копии элементов исходной последовательности.
        /// </returns>
        public static ObservableCollection<T> ToCollection<T>(this IEnumerable<T> source)
            where T : class
        {
            return ToCollection<T, T>(source);
        }

        /// <summary>
        /// Преобразует последовательность <see cref="IEnumerable{T}"/> в <see cref="ObservableCollection{TCollectionItem}"/>.
        /// Для каждого элемента создаётся новый экземпляр <typeparamref name="TCollectionItem"/>.
        /// </summary>
        /// <typeparam name="T">Тип элементов исходной последовательности.</typeparam>
        /// <typeparam name="TCollectionItem">Тип элементов результирующей коллекции.</typeparam>
        /// <param name="source">Исходная последовательность.</param>
        /// <param name="action">
        /// Пользовательское действие для заполнения элемента коллекции.
        /// Если <c>null</c>, используется копирование свойств из исходного объекта через <c>Obj.Copy</c>.
        /// </param>
        /// <returns>
        /// Коллекция <see cref="ObservableCollection{TCollectionItem}"/>, заполненная элементами,
        /// созданными на основе исходной последовательности.
        /// </returns>
        public static ObservableCollection<TCollectionItem> ToCollection<T, TCollectionItem>(this IEnumerable<T> source, Action<TCollectionItem, T> action = null)
            where T : class
            where TCollectionItem : class
        {
            var collection = new ObservableCollection<TCollectionItem>();
            foreach (var item in source)
            {
                var collectionItem = Obj.New<TCollectionItem>();
                if (action != null)
                {
                    action(collectionItem, item);
                }
                else
                {
        ...
        /// <summary>
        /// Преобразует последовательность <see cref="IEnumerable{T}"/> в <see cref="ObservableCollectionEx{T}"/>.
        /// Для каждого элемента создаётся новый объект и копируются его свойства.
        /// </summary>
        /// <typeparam name="T">Тип элементов исходной последовательности и коллекции.</typeparam>
        /// <param name="source">Исходная последовательность.</param>
        /// <returns>
        /// Коллекция <see cref="ObservableCollectionEx{T}"/>, содержащая копии элементов исходной последовательности.
        /// </returns>
        public static ObservableCollectionEx<T> ToCollectionEx<T>(this IEnumerable<T> source)
            where T : class
        {
            return ToCollectionEx<T, T>(source);
        }

        /// <summary>
        /// Преобразует последовательность <see cref="IEnumerable{T}"/> в <see cref="ObservableCollectionEx{TCollectionItem}"/>.
        /// Для каждого элемента создаётся новый экземпляр <typeparamref name="TCollectionItem"/>.
        /// </summary>
        /// <typeparam name="T">Тип элементов исходной последовательности.</typeparam>
        /// <typeparam name="TCollectionItem">Тип элементов результирующей коллекции.</typeparam>
        ...
        /// </param>
        /// <returns>
        /// Коллекция <see cref="ObservableCollectionEx{TCollectionItem}"/>, заполненная элементами,
        /// созданными на основе исходной последовательности.
        /// </returns>
        public static ObservableCollectionEx<TCollectionItem> ToCollectionEx<T, TCollectionItem>(this IEnumerable<T> source, Action<TCollectionItem, T> action = null)
            where T : class
            where TCollectionItem : class
        {
        ...
        /// <summary>
        /// Пытается получить элемент последовательности по указанному индексу.
        /// </summary>
        /// <typeparam name="T">Тип элементов последовательности.</typeparam>
        /// <param name="e">Последовательность, из которой необходимо получить элемент.</param>
        /// <param name="index">Индекс элемента.</param>
        /// <returns>
        /// Элемент последовательности с указанным индексом, если он существует;
        /// в противном случае — значение по умолчанию для типа <typeparamref name="T"/>.
        /// </returns>
        /// <exception cref="ArgumentNullException">
        /// Выбрасывается, если <paramref name="e"/> равен <c>null</c>.
        /// </exception>
        /// <remarks>
        /// Если последовательность реализует интерфейс <see cref="IList{T}"/>,
        /// используется прямой доступ по индексу для повышения производительности.
